// lib/acquiflow/marketFacts.ts
// ─────────────────────────────────────────────────────────────────────────────
// AcquiFlow — Market Facts Reader (canonical location)
//
// Returns ONLY raw factual market/evidence context. Queries the benchmark
// tables directly; never calls operational /api/benchmark-lookup. Returns
// honest absence ("unavailable") when no rows match — NEVER fabricates,
// synthesizes, or estimates benchmark values from the deal's own multiple.
//
// CONSUMERS (Patch C onward):
//   1. /api/deals/[id]/committee → Investment Memo (Intel page)
//   2. /api/reports/[dealId]      → Workspace PDF deal report
//
// Both reports MUST converge on the read returned by this function. That
// guarantee — single source, two consumers — is Invariant 1 applied to the
// benchmark layer (the same discipline the Tax workspace enforces).
//
...
//
// NOTE (Patch C, deferred policy): the current tier thresholds are
// size-band ≥ 5 samples and national ≥ 10 samples. Raising the size-band
// threshold to ≥ 10 for institutional rigor is a deliberate FUTURE policy
// change. It is intentionally not bundled here — the immediate goal is for
// both reports to consume identical logic. Threshold review is a separate
// product decision.
// ─────────────────────────────────────────────────────────────────────────────
...
async function readClosedComps(
  ...
  // Threshold policy review is a separate FUTURE decision (see header note).
