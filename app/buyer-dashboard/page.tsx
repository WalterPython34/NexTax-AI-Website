      ...
      <Card style={{ padding: 0, overflow: "hidden" }}>
        ...
        {!loading && filtered.length === 0 && (
          <div style={{ padding: "60px 24px", textAlign: "center" as const }}>
            ...
            <div style={{ fontSize: 16, fontWeight: 700, color: "#E2E8F0", marginBottom: 8, fontFamily: "'Inter Tight',sans-serif" }}>
              {search ? "No deals match your search." : "No deals yet."}
            </div>
            {!search ? (
              <>
                <div style={{ fontSize: 13, color: "#4B5563", lineHeight: 1.8, marginBottom: 20 }}>
                  Run your first analysis to see:<br />
                  <span style={{ color: "#6B7280" }}>
                    · True earnings (adjusted SDE){"  "}· Deal risk score<br />
                    · Market comps & valuation{"  "}· Negotiation strategy
                  </span>
                </div>
                <button
                  onClick={() => onAnalyzeNew()}
              ...
              <div style={{ fontSize: 12, color: "#4B5563" }}>Try a different search term.</div>
            )}
          </div>
        )}eact, { useState, useEffect, useCallback } from "react";
import { createClient } from "@supabase/supabase-js";
import { CATEGORIES } from "@/lib/marketview/categories";
import { normalizeDealFinancials } from "@/lib/normalizationEngine";
import { BlurGateSection, ProLockedBanner } from "@/components/BlurGateSection";
