-- StartSmart Supabase Schema Setup
...
CREATE POLICY "Users can manage own business profiles" ON business_profiles FOR ALL USING (auth.uid()::text = "userId");
CREATE POLICY "Users can manage own conversations" ON conversations FOR ALL USING (auth.uid()::text = "userId");
CREATE POLICY "Users can manage own messages" ON messages FOR ALL USING (auth.uid()::text IN (SELECT "userId" FROM conversations WHERE id = "conversationId"));
CREATE POLICY "Users can manage own tasks" ON progress_tasks FOR ALL USING (auth.uid()::text = "userId");
CREATE POLICY "Users can manage own documents" ON documents FOR ALL USING (auth.uid()::text = "userId");
CREATE POLICY "Users can manage own AI usage" ON ai_usage FOR ALL USING (auth.uid()::text = "userId");
...
COMMENT ON TABLE users IS 'Shared user table with NexTax.AI authentication system';
COMMENT ON TABLE user_subscriptions IS 'Shared subscription status table with NexTax.AI billing system';
COMMENT ON TABLE sessions IS 'Session storage for authentication persistence';