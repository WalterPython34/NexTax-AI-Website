...
export function useDealSummary(
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      try {
        // The Supabase session is stored in localStorage (not cookies), so we
        // must attach the access token explicitly as a Bearer header — the
        // operational-intelligence API tier authenticates via verified token.
        const headers: Record<string, string> = {};
        try {
          const raw = window.localStorage.getItem(
            "sb-sgrosezedxunoicmglpj-auth-token",
          );
          if (raw) {
            const parsed = JSON.parse(raw);
            const token =
              (Array.isArray(parsed) ? parsed[0]?.access_token : parsed?.access_token) ??
              (typeof parsed?.[0] === "string" ? parsed[0] : null);
            if (token) headers["Authorization"] = `Bearer ${token}`;
          }
        } catch {
          /* no token available; request will 401 with a diagnostic reason */
        }

        const res = await fetch(urlStr, { method: "GET", headers });
