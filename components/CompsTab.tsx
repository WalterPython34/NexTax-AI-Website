...
//
// Data contract:
//   - All values pre-computed by caller (no business logic here)
//   - percentile values are optional — component degrades gracefully
//   - benchmarkIsProxy drives proxy chip visibility
//   - normalization section only renders when reported ≠ adjusted
...
// ═══════════════════════════════════════════════════════════════════════════════
// DESIGN TOKENS — matches buyer-dashboard.tsx and UnderwritingPanel.tsx exactly
// ═══════════════════════════════════════════════════════════════════════════════
...
const T = {
  ...
  bgHover:     "rgba(255,255,255,0.032)",
  ...
  violet:      "#A78BFA",  violetBg: "rgba(167,139,250,0.07)",  violetBd: "rgba(167,139,250,0.2)",
  slate:       "#64748B",  slateBg:  "rgba(100,116,139,0.07)",  slateBd:  "rgba(100,116,139,0.2)",
};

// ═══════════════════════════════════════════════════════════════════════════════
// HELPERS — formatting only, zero business logic
// ═══════════════════════════════════════════════════════════════════════════════
...
export interface ComparableDeal {
  id:         string;
  name:       string;           // e.g. "Southeast Landscaping Co."
  revenue:    number;
  sde:        number;
  multiple:   number;
  location?:  string | null;    // e.g. "Austin, TX"
  note?:      string | null;    // e.g. "SBA-financed", "absentee owner"
}
...
export interface MarketPositionHeroProps {
  benchmarkLabel:    string;        // e.g. "Restaurant & Food Benchmark"
  benchmarkIsProxy:  boolean;
  pricingLabel:      PricingLabel;
  percentile?:       number | null; // 0–100, optional until populated
...
export interface RangeBarData {
  ...
  note?:        string;
  isAdjusted?:  boolean; // true when value comes from normalization
}
...
export interface ReportedVsAdjustedSectionProps {
  ...
  isAdjusted:          boolean; // false = section is collapsed / hidden
}
...
export interface BenchmarkContext {
  ...
  medianMultiple:   number;
  highMultiple:     number;
  marginLow:        number;    // decimal
  marginMedian:     number;    // decimal
  marginHigh:       number;    // decimal
  dscrLow:          number;
  dscrMedian:       number;
  dscrHigh:         number;
}
...
export interface NormalizationContext {
  ...
  earningsSource:      "reported" | "blended" | "benchmark_implied";
  isAdjusted:          boolean;
  currentMargin:       number | null; // usable_sde / revenue
  currentDscr:         number;
}
...
// ── Pricing label → color mapping ────────────────────────────────────────────
function pricingColor(label: PricingLabel): string {
  switch (label) {
    case "Well Below Market":          return T.green;
...
function pricingBg(label: PricingLabel): string {
  switch (label) {
    case "Well Below Market":          return T.greenBg;
    ...
    <SectionCard style={{ marginBottom: 10 }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
        <div style={{ display: "flex", gap: 5, flexWrap: "wrap" as const }}>
          ...
          {benchmarkIsProxy && (
            <Chip label="Proxy Benchmark" color={T.amber} bg={T.amberBg} bd={T.amberBd} />
          )}
          {benchmarkSource === "direct" && (
            <Chip label="Direct RMA" color={T.green} bg={T.greenBg} bd={T.greenBd} />
          )}
          {benchmarkSource === "fallback" && (
            <Chip label="Fallback Estimate" color={T.slate} bg={T.slateBg} bd={T.slateBd} />
          )}
        </div>

        {/* Percentile badge — right side, optional */}
        {percentile != null ? (
          <div style={{
            display:       "flex",
            ...
            <div style={{ fontSize: 9, color: T.textMuted, textTransform: "uppercase" as const, letterSpacing: "0.08em", marginBottom: 2 }}>
              Percentile
            </div>
      ...
      <div style={{ fontSize: 11, color: T.textSub, lineHeight: 1.5 }}>
        <Mono color={T.text} size={11}>{formatMultiple(currentMultiple)}</Mono>
...
export function RangeBar({
  ...
  // Position math — clamp to [0, 100] for display
  const rangeSpan    = high - low;
  const trackPadPct  = 10; // visual breathing room on each end of track
  const usableWidth  = 100 - 2 * trackPadPct;

  function toPosition(v: number): number {
    if (rangeSpan <= 0) return 50;
    const raw = ((v - low) / rangeSpan) * usableWidth + trackPadPct;
  ...
  const markerPos  = currentValue != null
    ? isOutLeft  ? 2
    : isOutRight ? 98
    : toPosition(currentValue)
    : null;
    ...
    <div style={{ paddingBottom: 14 }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 6 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          ...
          {isAdjusted && (
            <Chip label="Adjusted" color={T.indigo} bg={T.indigoBg} bd={T.indigoBd} />
          )}
          {isOutRange && (
            <Chip label="Outside Range" color={T.red} bg={T.redBg} bd={T.redBd} />
          )}
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <span style={{ fontSize: 10, color: T.textMuted }}>
            {formatValue(low, format)} — {formatValue(high, format)}
          </span>
          {currentValue != null && (
            <Mono color={markerColor} size={12}>{formatValue(currentValue, format)}</Mono>
          )}
        </div>
      </div>
      ...
      {/* Optional note */}
      {note && (
        <div style={{ fontSize: 10, color: T.textMuted, marginTop: 3, lineHeight: 1.4 }}>{note}</div>
      )}
    </div>
  );
}
...
export function RangeBarsSection({ bars }: RangeBarsSectionProps) {
  return (
    <SectionCard style={{ marginBottom: 10 }}>
      <SectionLabel>Market Range Comparison</SectionLabel>
      <div>
        {bars.map((bar, i) => (
          <div
            key={bar.label}
...
export function ReportedVsAdjustedSection({
  ...
  return (
    <SectionCard style={{ marginBottom: 10 }}>
      <SectionLabel>Normalization Adjustment</SectionLabel>
      ...
      <div style={{
        ...
        Market positioning uses trust-adjusted earnings when reported financials appear unreliable.
        {" "}Usable SDE is{" "}
        <span style={{ color: T.indigo, fontFamily: T.mono, fontWeight: 700 }}>
          {srcLabel.toLowerCase()}
        </span>
        {delta != null && (
          <span style={{ color: Math.abs(delta) > 20 ? T.orange : T.textSub }}>
            {" — "}{Math.abs(delta)}% {delta < 0 ? "below" : "above"} reported
          </span>
        )}
        .
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 0 }}>
        ...
        <CRow
          label="SDE"
          reported={formatMoney(reportedSDE)}
          adjusted={formatMoney(usableSDE)}
          adjustedColor={T.indigo}
          changed={reportedSDE !== usableSDE}
        />

        {/* Fair value row */}
        <CRow
          label="Fair Value"
          reported={formatMoney(reportedFairValue)}
          adjusted={formatMoney(adjustedFairValue)}
          adjustedColor={T.green}
          changed={reportedFairValue !== adjustedFairValue}
        />

        {/* Benchmark-implied row if available */}
        {benchmarkImpliedSDE != null && (
          <CRow
            label="Benchmark SDE"
            reported="—"
            adjusted={formatMoney(benchmarkImpliedSDE)}
            adjustedColor={T.textSub}
            changed={false}
          />
        )}
      </div>
...
function CRow({
  ...
  return (
    <>
      <div style={{ fontSize: 11, color: T.textSub, padding: "9px 0", borderBottom: `1px solid ${T.borderLight}` }}>
        {label}
      </div>
      <div style={{
        fontSize:     12,
        fontFamily:   T.mono,
        fontWeight:   600,
        color:        changed ? T.textMuted : T.text,
...
// ═══════════════════════════════════════════════════════════════════════════════
// 4. ComparableDealsSection
// ═══════════════════════════════════════════════════════════════════════════════

export function ComparableDealsSection({
  comps,
  currentMultiple,
  currentDealOutsideRange,
  benchmarkIsProxy,
}: ComparableDealsSectionProps) {
  if (comps.length === 0) {
    return (
      <SectionCard style={{ marginBottom: 10 }}>
    ...
    <SectionCard style={{ marginBottom: 10 }}>
      ...
      <div style={{ overflowX: "auto" as const }}>
        <table style={{ width: "100%", borderCollapse: "collapse" as const }}>
          <thead>
            <tr>
              {(["Deal", "Revenue", "SDE", "Multiple", "Note"] as const).map((h, i) => (
                <th key={h} style={{
                  fontSize:      9,
                  fontWeight:    700,
                  color:         T.textMuted,
          ...
          <tbody>
              ...
              return (
                <tr key={comp.id}>
                  ...
                  <td style={{
                    ...
                    {comp.location && (
                      <div style={{ fontSize: 9, color: T.textMuted, marginTop: 1 }}>{comp.location}</div>
                    )}
                  </td>
                  {/* Revenue */}
                  <td style={{
                  ...
                  <td style={{
                    ...
                    {formatMoney(comp.sde)}
                  </td>
                  {/* Multiple — colored by relative position */}
                  <td style={{
                    fontSize:    12,
                    fontFamily:  T.mono,
                    fontWeight:  700,
                    color:       isLowest ? T.green : isHighest ? T.orange : T.text,
                    textAlign:   "right" as const,
                    padding:     "8px 12px 8px 0",
                    borderBottom:`1px solid ${T.borderLight}`,
                    whiteSpace:  "nowrap" as const,
            ...
            {/* Current deal row — highlighted */}
            <tr>
              <td style={{ fontSize: 11, fontWeight: 700, color: T.indigo, padding: "9px 12px 0 0" }}>
                This Deal
              </td>
              ...
              <td style={{
                ...
                fontFamily: T.mono,
                fontWeight: 800,
                color:      currentDealOutsideRange ? T.red : T.indigo,
                textAlign:  "right" as const,
                ...
                {currentDealOutsideRange && (
                  <span style={{ fontSize: 9, marginLeft: 5, color: T.red }}>↑ outside range</span>
                )}
              </td>
      ...
      <div style={{
        ...
        borderTop:   `1px solid ${T.borderLight}`,
        fontSize:    10,
        color:       currentDealOutsideRange ? T.orange : T.textMuted,
        lineHeight:  1.5,
      }}>
        {currentDealOutsideRange
...
export function CompsTab({
  ...
  const {
    benchmarkLabel, benchmarkIsProxy, benchmarkSource,
    lowMultiple, medianMultiple, highMultiple,
    marginLow, marginMedian, marginHigh,
    dscrLow, dscrMedian, dscrHigh,
  } = benchmarkContext;

  const { pricingLabel, percentile, currentMultiple } = marketPosition;

  // Build range bars from context + normalization
  const bars: RangeBarData[] = [
    {
      ...
      note: normalization.isAdjusted
        ? `Based on usable SDE (${formatMoney(normalization.usableSDE)}) — reported SDE was ${formatMoney(normalization.reportedSDE)}`
        : undefined,
    },
    {
      label:        "SDE Margin",
    ...
    },
    {
      ...
      note: normalization.currentDscr < 1.0
        ? "DSCR below 1.0x — deal cannot service debt at current terms"
        : normalization.currentDscr < 1.25
        ? "DSCR below 1.25x lender minimum — financing at risk"
        : undefined,
    },
  ];
