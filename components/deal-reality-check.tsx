...
function ScoreBar({ multiple, heroColor }: { multiple: number; heroColor: string }) {
  const markerX = Math.min(290, Math.max(10,
    multiple < 1.0  ? 10 + (multiple / 1.0) * 48 :
    multiple < 1.15 ? 60 + ((multiple - 1.0) / 0.15) * 58 :
    multiple < 1.30 ? 120 + ((multiple - 1.15) / 0.15) * 72 :
    194 + Math.min(96, ((multiple - 1.30) / 0.5) * 96)
  ));
  return (
    <div style={{ marginTop: 16 }}>
      <svg viewBox="0 0 300 28" xmlns="http://www.w3.org/2000/svg" style={{ width: "100%", height: 28, display: "block" }}>
        ...
        <text x={markerX} y="18" fontSize="7" fill="white" textAnchor="middle" fontWeight="500" fontFamily="sans-serif">{multiple.toFixed(2)}</text>
      </svg>
    </div>
  );
}
